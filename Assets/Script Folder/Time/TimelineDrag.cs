...
public class TimelineDrag : MonoBehaviour
{
    ...
    [SerializeField] private Transform leftTargetObject;//获取左边边界的位置
    ...
    private void Update()
    {
       ...
       // Reset();//这个函数，是为了防止玩家将时间轴拖到了范围外，如果拖到了范围外，将强行重置到两个边界
        ...
        if (isSelected && DistanceJudge)//如果鼠标超出了边界，那么不再进行可拖拽互动
        {
            // 获取当前鼠标位置
    ...
    // 在“Time Marker”的当前位置生成一个新的Marker，在UI界面，它会出现一个小三角形
    // private void CreateMarkerAtCurrentPosition()
    // {
    //     if (markerPrefab != null && camParent != null)
    //     {

    //     // 设置y轴方向的偏移量，这是为了生成的标记能和“小眼睛”错开
    //     float yOffset = -2f; // 您可以根据需要调整这个值
    //     Vector3 newPosition = transform.position;
    //     newPosition.y += yOffset; // 在y轴方向上向下偏移

    //     GameObject newMarker = Instantiate(markerPrefab, newPosition, Quaternion.identity, camParent);
    //     // 可以设置Marker的其他属性，如颜色、大小等
        
    //     temporaryDistance = newPosition.x - leftTargetObject.position.x;
    //     percentage =  (temporaryDistance / maxDistance) * 100; // 计算百分比
    //     Debug.Log("这个位置占总长度的"+percentage+"%"); // 输出百分比
    //     }
    // }
...
/// </summary>
    private void CalculateMouseXDistance()    // 计算鼠标和左边边界的距离
    {
        ...
        // 输出到控制台，或者根据需要进行其他操作
        // Debug.Log("The x-axis distance between the mouse and the target object is: " + xDistance);
    }
